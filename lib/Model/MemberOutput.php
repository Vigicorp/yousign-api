...
class MemberOutput implements ModelInterface, ArrayAccess
{
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['user']) && !preg_match("/^\/users\/[0-9a-f]{8}-([0-9a-f]{4}-){3}[0-9a-f]{12}$/", $this->container['user'])) {
            $invalidProperties[] = "invalid value for 'user', must be conform to the pattern /^\/users\/[0-9a-f]{8}-([0-9a-f]{4}-){3}[0-9a-f]{12}$/.";
        }
    ...
    public function setUser($user)
    {
        ...
        if (!is_null($user) && (!preg_match("/^\/users\/[0-9a-f]{8}-([0-9a-f]{4}-){3}[0-9a-f]{12}$/", $user))) {
            throw new \InvalidArgumentException("invalid value for $user when calling MemberOutput., must conform to the pattern /^\/users\/[0-9a-f]{8}-([0-9a-f]{4}-){3}[0-9a-f]{12}$/.");
        }
